require 'roda'
require_relative '../../core_ext/hash'   unless Hash.new.respond_to?(:to_html_attributes)
  ...
  module RodaPlugins
    ...
    module RodaTags
      ...
      module InstanceMethods
        ...
        # Update the <tt>:class</tt> entry in the +attr+ hash with the given +classes+ and returns +attr+.
        # 
        #   attr = { class: 'alert', id: :idval }
        #   
        #   merge_attr_classes(attr, 'alert-info')  #=> { class: 'alert alert-info', id: :idval }
        # 
        #   merge_attr_classes(attr, [:alert, 'alert-info'])  #=> { class: 'alert alert-info', id: :idval }
        # 
        ...
        def block_is_template?(block) 
           block && (is_erb_template?(block) || (self.respond_to?(:block_is_haml?) && block_is_haml?(block)))
        end
        ...
        private
          ...
          def tag_contents_for(name, content, newline = nil )
            if multi_line_tag?(name)
              "#{add_newline?(newline)}#{content.to_s}#{add_newline?(newline)}".gsub(/\n\n/,"\n")
          ...
          def normalize_html_attributes(attrs = {}) 
            ...
            if value = attrs.delete(:data)  
              value.each { |k,v| attrs[:"data-#{k.to_s}"] = v }  #NB!! convert key to symbol for [].sort
            end
          ...
          alias_method :is_erb_buffer?, :have_buffer?

          #
          def is_erb_block?(block)
            have_buffer? || block && eval('defined? __in_erb_template', block.binding)
          end
          alias_method :is_erb_template?, :is_erb_block?
