      ...
      describe ':tags opts' do
        ...
        describe 'defaults' do
          ...
          it 'should have the default settings' do
            ...
            @a.tags_opts.must_equal(expected)
          end
      
          describe '#:tag_output_format_is_xhtml' do
        
            it 'should return false' do
              tag_app('<%= output_is_xhtml?.inspect %>').must_equal "false"
              tag_app('<%= output_is_xhtml?.inspect %>', {}, { tag_output_format_is_xhtml: false }).must_equal "false"
            end
        ...
        describe 'custom settings' do
          ...
          it 'should have the custom settings' do
            ...
            @b.tags_opts.must_equal(expected)
          end
          ...
          describe '#:tag_output_format_is_xhtml' do
            ...
            it 'should return true' do
              tag_app('<%= output_is_xhtml?.inspect %>', {}, { tag_output_format_is_xhtml: true }).must_equal "true"
            end
        ...
        describe 'double loading of plugin' do
          ...
          it 'should have the custom settings' do
            ...
            @d.tags_opts.must_equal(expected)
          end
      ...
      describe 'Instance Methods' do
        ...
        describe '#tag' do
          ...
          describe "multi line tags " do 
            ...
            Roda::RodaPlugins::RodaTags::MULTI_LINE_TAGS.each do |t|
              ...
              describe "like <#{t}>" do 
                ...
                it "should have a '\\n' after the opening tag and before the closing tag" do 
                  tag_app("<%= tag(:#{t},'contents') %>").must_equal "<#{t}>\ncontents\n</#{t}>\n"
                end
          
                it "should work without contents passed in" do 
                  tag_app("<%= tag(:#{t},nil) %>").must_equal "<#{t}>\n</#{t}>\n"
                end
          
                it "should allow a hash of attributes to be passed" do 
                  tag_app("<%= tag(:#{t},'contents', id: 'tid', class: 'klass') %>")
                ...
                it "with ':newline => false' should NOT add '\\n' around the contents" do 
                  tag_app("<%= tag(:#{t},'content', :id => 'tag-id', :newline => false) %>")
            ...
            describe "like <textarea>" do 
              ...
              it "should NOT have a '\\n' after the opening tag and before the closing tag" do 
                tag_app("<%= tag(:textarea,'contents') %>").must_equal "<textarea>contents</textarea>\n"
              end
        
              it "should work without contents passed in" do 
                tag_app("<%= tag(:textarea,nil) %>").must_equal "<textarea></textarea>\n"
              end
              ...
              it "with ':newline => false' should NOT add '\\n' around the contents" do 
                tag_app("<%= tag(:textarea,'content', id: 'tag-id', newline: false) %>")
            ...
            Roda::RodaPlugins::RodaTags::SELF_CLOSING_TAGS.each do |t|
              ...
              describe "like <#{t}>" do 
                ...
                it "should add a '/' to close the tag if format is XHTML" do 
                  tag_app("<%= tag(:#{t}) %>", {}, { tag_output_format_is_xhtml: true } ).must_equal "<#{t} />\n"
                end
          ...
          it 'should return the correct <br> tag' do
            tag_app('<%= tag(:br) %>').must_equal "<br>\n"
            tag_app('<%= tag(:br) %>', {}, { tag_output_format_is_xhtml: true } ).strip.must_equal "<br />"
          end
    
          it 'should return the correct <div> tag' do
            tag_app("<%= tag(:div) %>").must_equal "<div>\n</div>\n"
          end
    
          it 'should return the correct <section> tag' do
            tag_app("<%= tag(:section) %>").must_equal "<section>\n</section>\n"
          end
    
          it 'should return the correct <input> tag' do
            tag_app("<%= tag(:input, type: :text, id: :idval, class: :klass) %>").must_equal %Q{<input class=\"klass\" id=\"idval\" type=\"text\">\n}
          end
    
          it 'should handle data hashes correctly' do
            tag_app("<%= tag(:div, id: 'ID', class: 'klass', data: { c: 'C', b: 'B', a: 'A' }) %>")
...
ERB
        ...
        describe '#merge_attr_classes' do
          ...
          it 'should merge the given classes' do
            tag_app('<%= merge_attr_classes({ class: "a b d" }, "c") %>').must_equal '{:class=>"a b c d"}' #"a b c d"
          end
    
          it 'should merge the given classes and return only uniq classes' do
            tag_app('<%= merge_attr_classes({ class: "a b d" }, "a b") %>').must_equal '{:class=>"a b d"}' #"a b d"
          end
        ...
        describe '#merge_classes' do
          ...
          it 'should correctly handle being passed empty arrays' do
            tag_app('<% @a={ class: [] } %><%= merge_classes(@a[:class], []) %>').must_equal ""
          end
          
          it 'should correctly handle being passed nil attrs and an empty array' do
            tag_app('<%= merge_classes(nil, []) %>').must_equal ""
          end
          
          it 'should correctly handle being passed nil values only' do
            tag_app('<%= merge_classes(nil, nil) %>').must_equal ""
          end
          
          it 'should correctly handle being passed empty hash values only' do
            tag_app('<%= merge_classes({}, {}) %>').must_equal ""
          end
    
          it 'should correctly handle being passed nil & empty array' do
            tag_app('<% @a={ class: nil   } %><%= merge_classes(@a[:class], []) %>').must_equal ""
            tag_app('<% @a={ class: nil   } %><%= merge_classes(@a[:class], nil) %>').must_equal ""
            tag_app('<% @a={ class: nil   } %><%= merge_classes(@a[:class], [nil]) %>').must_equal ""
            tag_app('<% @a={ class: [nil] } %><%= merge_classes(@a[:class], nil) %>').must_equal ""
            tag_app('<% @a={ class: [nil] } %><%= merge_classes(@a[:class], [nil]) %>').must_equal ""
            tag_app('<% @a={ class: [nil] } %><%= merge_classes(@a[:class], [:alert]) %>').must_equal "alert"
            tag_app('<% @a={ class: [nil] } %><%= merge_classes(@a[:class], [:alert, nil]) %>').must_equal "alert"
          end
    
          it 'should correctly handle removing duplicate values' do
            tag_app('<%= merge_classes({class: :alert }, :alert) %>').must_equal "alert"
            tag_app('<%= merge_classes({class: :alert }, [:alert]) %>').must_equal "alert"
            tag_app('<%= merge_classes({class: [:alert] }, :alert) %>').must_equal "alert"
            tag_app('<%= merge_classes({class: [:alert] }, [:alert]) %>').must_equal "alert"
            tag_app('<%= merge_classes({class: "alert" }, :alert) %>').must_equal "alert"
          ...
          it 'should correctly merge the given classes when passed an empty array & array[strings]' do
            tag_app('<% @a={ class: [] } %><%= merge_classes(@a[:class], ["alert", "alert-info"]) %>').must_equal "alert alert-info"
          end
    
          it 'should correctly merge the given classes when passed nil & one with array[strings]' do
            tag_app('<% @a={ class: nil   } %><%= merge_classes(@a[:class], ["alert", "alert-info"]) %>').must_equal "alert alert-info"
            tag_app('<% @a={ class: [nil] } %><%= merge_classes(@a[:class], ["alert", "alert-info"]) %>').must_equal "alert alert-info"
          end
    
          it 'should correctly merge the given classes when passed arrays with strings' do
            tag_app('<% @a={ class: ["alert"] } %><%= merge_classes(@a[:class], ["alert", "alert-info"]) %>').must_equal "alert alert-info"
          end
    
          it 'should correctly merge the given classes when passed string & array' do
            tag_app('<% @a={ class: "alert" } %><%= merge_classes(@a[:class], ["alert", "alert-danger"]) %>').must_equal "alert alert-danger"
          end
          ...
          it 'should correctly merge the given classes when passed array[symbol] & array' do
            tag_app('<% @a={ class: [:alert, "error"] } %><%= merge_classes(@a[:class], ["alert", "alert-danger"]) %>').must_equal "alert alert-danger error"
          end
    
          it 'should correctly merge the given classes when passed array[symbol] & :symbol' do
            tag_app('<% @a={ class: [:alert] } %><%= merge_classes(@a[:class], :text) %>').must_equal "alert text"
          end
          ...
          it 'should correctly merge the given classes when passed :symbol & :symbol' do
            tag_app('<% @a={ class: :alert } %><%= merge_classes(@a[:class], :text) %>').must_equal "alert text"
            tag_app('<% @a={ class: :"alert-info" } %><%= merge_classes(@a[:class], :text) %>').must_equal "alert-info text"
          end
        ...
        describe '#capture' do
          ...
          it "should capture the whitespace of the block but not the silent '<% tag(:br) %>' tag output" do
            tag_app(%Q{<% capture() do %> <% tag(:br) %> <% end %>}).must_equal '  '
          end
          
          it "should capture the whitespace of the block including the '<%= tag(:br) %>' output" do
            tag_app(%Q{<% capture() do %>|<%= tag(:br) %>|<% end %>}).must_equal %Q{|<br>\n|}
          end
...
ERB
            html = tag_app(str)
            html.must_have_tag('div.row > div[@class="col-md-12"] > h1', 'Capture Works too')
            html.strip.must_have_tag('div.row > div[@class="col-md-12"] > h2', %Q{        Nested capture works too \n})
          end
        ...
        describe '#normalize_html_attributes' do
          ...
          it 'should handle empty attrs' do
            tag_app('<%= normalize_html_attributes({}) %>').must_equal ""
          end
    
          it 'should handle basic attrs' do
            tag_app('<%= normalize_html_attributes({class: "alert"}) %>').must_equal ' class="alert"'
          end
    
          it 'should handle data attrs' do
            tag_app('<%= normalize_html_attributes({data: { b: :B, a: :A }, class: "alert"}) %>')
              .must_equal ' class="alert" data-a="A" data-b="B"'
          end
    
          it 'should handle boolean attrs' do
            tag_app('<%= normalize_html_attributes({data: { b: :B, a: :A }, class: "alert", selected: true}) %>')
              .must_equal ' class="alert" data-a="A" data-b="B" selected="selected"'
            tag_app('<%= normalize_html_attributes({data: { b: :B, a: :A }, class: "alert", selected: false}) %>')
              .must_equal ' class="alert" data-a="A" data-b="B"'
          end
        ...
        describe '#concat_content' do
          ...
          it 'should handle within a block' do
            ...
            # @r.concat_content('text').must_equal ''
            # @r.app.must_be_kind_of(Roda)
            # @b.new.concat_content("text").must_equal ''
